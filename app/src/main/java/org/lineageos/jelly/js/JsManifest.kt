...
class JsManifest(
    ...
    fun onResolved(url: String, value: String) {
        ...
        getIconUrl(url, manifest)?.let {
            scope.launch {
                val bitmap = getIconBitmap(it)
                if (bitmap != null) {
    ...
    private fun getIconUrl(baseUrl: String, manifest: JSONObject): String? {
        var url: String? = null
        var minWidth = 33
        val maxWidth = 333
        ...
        return if (url != null) URI(baseUrl).resolve(url).toString() else null
    }
