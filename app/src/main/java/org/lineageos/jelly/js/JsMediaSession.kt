...
class JsMediaSession(private val webView: WebViewExt) {
    ...
    companion object {
        ...
        private const val DESTROY_OBSERVER = "DestroyObserver"
        ...
        const val SCRIPT = """
            (() => {
                ...
                const beforeDestroy = () => {
                    if (!$MEDIA) return;
                    if ($MEDIA.$DESTROY_OBSERVER) {
                        $MEDIA.$DESTROY_OBSERVER.disconnect();
                        delete $MEDIA.$DESTROY_OBSERVER;
                    }
                    $MEDIA.onpause = () => {};
                    $PAUSE_ACTION();
                };
                ...
                const onCreate = (media, event) => {
                    ...
                    if (media.parentNode) {
                        media.$DESTROY_OBSERVER = new MutationObserver((mutationsList) => {
                            for (const mutation of mutationsList) {
                                mutation.removedNodes.forEach((node) => {
                                    if (node === media) $DESTROY_ACTION();
                                });
                            }
                        });
                        media.$DESTROY_OBSERVER.observe(media.parentNode, {childList: true});
                    }
                    beforeDestroy();
                    $MEDIA = media;
