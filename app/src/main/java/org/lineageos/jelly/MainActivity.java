...
public class MainActivity extends WebViewExtActivity implements View.OnTouchListener,
    ...
    private void setupSearch() {
        ...
        mWebView.setFindListener((activeMatchOrdinal, numberOfMatches, isDoneCounting) -> {
            boolean searchSuccessful = numberOfMatches > 0;
            // Enable prev/next if matches are more than 0
            UiUtils.setImageButtonEnabled(this, searchSuccessful, searchMenuPrev);
            UiUtils.setImageButtonEnabled(this, searchSuccessful, searchMenuNext);
            // Update status text only when search is done
            if (searchSuccessful && isDoneCounting) {
        ...
        UiUtils.setImageButtonEnabled(this, false, searchMenuPrev);
        searchMenuNext.setOnClickListener(v -> {
            UiUtils.hideKeyboard(searchMenuEdit);
            mWebView.findNext(true);
        });
        UiUtils.setImageButtonEnabled(this, false, searchMenuNext);
