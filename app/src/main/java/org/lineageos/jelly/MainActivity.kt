...
class MainActivity : AppCompatActivity(), SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val isIncognito = savedInstanceState?.getBoolean(
            IntentUtils.EXTRA_INCOGNITO, false
        ) ?: intent.getBooleanExtra(IntentUtils.EXTRA_INCOGNITO, false)

        val url = intent.dataString?.takeIf {
            it.isNotEmpty()
        } ?: savedInstanceState?.getString(IntentUtils.EXTRA_URL, null)
        ...
        menuDialog = MenuDialog(this) { option: MenuDialog.Option ->
            when (option) {
                ...
                MenuDialog.Option.DESKTOP_VIEW -> {
                    webView.desktopMode = !webView.desktopMode
                }
        ...
        webView.init(isIncognito)
        webView.desktopMode = savedInstanceState?.getBoolean(
            IntentUtils.EXTRA_DESKTOP_MODE, false
        ) ?: false
        ...
        model.onShowSheetMenu.observe(this) {
            showSheetMenu(it.url, it.shouldAllowDownload)
        }

        // Observe download requests
        model.onDownloadStart.observe(this) {
            downloadFileAsk(
                it.url, it.userAgent, it.contentDisposition, it.mimeType, it.contentLength
            )
        }

        // Observe file chooser requests
        model.onShowFileChooser.observe(this) {
    ...
    public override fun onSaveInstanceState(outState: Bundle) {
        ...
        outState.putBoolean(IntentUtils.EXTRA_INCOGNITO, model.isIncognito.value == true)
        outState.putBoolean(IntentUtils.EXTRA_DESKTOP_MODE, model.desktopMode.value == true)
    }
