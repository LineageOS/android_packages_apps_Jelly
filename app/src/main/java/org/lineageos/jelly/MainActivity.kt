...
import org.lineageos.jelly.utils.openInNewTab
...
class MainActivity : WebViewExtActivity(), SearchBarController.OnCancelListener, SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        autoCompleteTextView.setOnKeyListener { _: View?, keyCode: Int, _: KeyEvent? ->
        ...
        autoCompleteTextView.onItemClickListener = OnItemClickListener { _: AdapterView<*>?, view: View, _: Int, _: Long ->
    ...
    private fun applyThemeColor(color: Int) {
        ...
        val progressColor = if (hasValidColor) if (UiUtils.isColorLight(localColor)) Color.BLACK else Color.WHITE else ContextCompat.getColor(this, R.color.colorAccent)
        ...
        flags = if (UiUtils.isColorLight(localColor)) {
            flags or if (isReachMode) View.SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR else View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR
        } else {
            flags and if (isReachMode) View.SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR.inv() else View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR.inv()
        }
    ...
    private class SetAsFavoriteTask internal constructor(private val contentResolver: ContentResolver, private val title: String, private val url: String,
                                                         private val color: Int, parentView: View?) : AsyncTask<Void?, Void?, Boolean>() {
