 ...
 */
internal abstract class SuggestionProvider {
     ...
     * *
     * @param addResult the Unit to invoke for each received suggestion
     * *
     * @throws Exception throw an exception if anything goes wrong.
     */
    @Throws(Exception::class)
    internal open fun parseResults(content: String,
    ...
    fun fetchResults(rawQuery: String): List<String> {
        ...
        val content = downloadSuggestionsForQuery(query, mLanguage) ?: // There are no suggestions for this query, return an empty list.
                return filter
        try {
            parseResults(content, { suggestion: String ->
    ...
    private fun getEncoding(connection: HttpURLConnection): String {
        ...
        return contentType.split(";".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
                .map { value -> value.trim { it <= ' ' } }
                .firstOrNull { it.toLowerCase(Locale.US).startsWith("charset=") }
                ?.substring(8)
                ?: mEncoding
    }
