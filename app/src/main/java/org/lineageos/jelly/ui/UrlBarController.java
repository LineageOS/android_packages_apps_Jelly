...
public class UrlBarController implements View.OnFocusChangeListener {
    ...
    public void onPageLoadFinished(Context context, SslCertificate certificate) {
        ...
        if (certificate != null) {
            // Make it so the dialog will show if you tap the lock icon and the cert is valid
            mSecureIcon.setOnClickListener((v) -> {
                ...
                String startDateLabel = context.getString(R.string.ssl_cert_dialog_valid_since);
                String endDateLabel = context.getString(R.string.ssl_cert_dialog_valid_until);
